...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if (strcmp(argv[3], "create") == 0)
            ...
            for (int i = 0; i < T_NUM; i++)
                fprintf(output, ",t%d_sys,t%d", i, i);
        fprintf(output, "\n");
    }
    ...
    for (int bn = 0; bn < B_NUM; bn++)
    {
        ...
        for (int i = 0; i < T_NUM; i++)
        {
            ...
            rtf_params_set_runtime(&p,
                rand_bounded(T_BUDGET_MIN, T_BUDGET_MAX));
            rtf_params_set_priority(&p,
                rand_bounded(T_PRIORI_MIN, T_PRIORI_MAX));
            ...
            if (strcmp(argv[3], "create") == 0)
                fprintf(output, ",%ld",
                    rdtsc_elapsed_ns(tsc_before, tsc_after));
#endif
