 ...
 * @internal
 * 
 * Receives @p size data from the socket contained in @p us and copies it in @p elem
 * buffer. Returns -1 in case of errors, 0 if server has closed the connection,
 * a positive value that indicates the number of bytes received in case of 
 * success
 * 
 * @endinternal
 */
...
int usocket_bind(struct usocket* us, char* filepath) {
    ...
    ret = bind(us->socket, (struct sockaddr*)&usock_sockaddr, sizeof(struct sockaddr_un));
 ...
 * @internal
 * 
 * Sends @p size data from one of the fd-set contained in @p us copying from @p elem
