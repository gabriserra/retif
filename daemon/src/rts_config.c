...
int rts_config_get_rt_kernel_max_util(float* rt_max_util)
{
    ...
    if (rts_config_get_rt_kernel_params(&rt_period, &rt_runtime) < 0)
        return -1;
...
int rts_config_restore_rt_kernel_params() 
{
    ...
    ret = rts_config_set_rt_kernel_params(PROC_RT_PERIOD_DEFAULT, PROC_RT_RUNTIME_DEFAULT);
 ...
 * @endinternal
 */
int rts_config_restore_rr_kernel_param(int rr_timeslice) 
{
    int ret;
    
    ret = rts_config_set_rr_kernel_param(PROC_RR_TIMESLICE_DEFAULT);
...
int rts_config_apply() 
{
    ...
    if (rts_config_set_rt_kernel_params(rt_period, rt_runtime) < 0)
        return -1;

    if (rts_config_set_rr_kernel_param(rr_timeslice) < 0)
        return -1;
